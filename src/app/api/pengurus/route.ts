...
export async function PUT(req: Request) {
  try {
    ...
    // Delete any existing officer account for this ukmId or email to prevent unique key conflicts
    await db.query('DELETE FROM pengurus_ukm WHERE ukm_id = $1 OR email = $2', [ukmId, cleanEmail]);

    // Insert fresh officer account cleanly
